  ...
  const getHeaders = () => {
    switch (tabType) {
      ...
      case 'behavior':
        return (
          <>
            <th className="px-6 py-5 text-xs font-bold text-[#8B95A1] uppercase tracking-wider">인성/생활 관찰</th>
            <th className="px-6 py-5 text-xs font-bold text-[#8B95A1] uppercase tracking-wider">교사 메모</th>
            <th className="px-6 py-5 text-xs font-bold text-[#8B95A1] uppercase tracking-wider">행동특성 문구</th>
            <th className="px-6 py-5 text-xs font-bold text-[#8B95A1] uppercase tracking-wider w-32">바이트수</th>
          </>
        );
      case 'subject':
        return (
          <>
            ...
            <th className="px-6 py-5 text-xs font-bold text-[#8B95A1] uppercase tracking-wider">교사 관찰</th>
            <th className="px-6 py-5 text-xs font-bold text-[#8B95A1] uppercase tracking-wider">세특 문구</th>
            <th className="px-6 py-5 text-xs font-bold text-[#8B95A1] uppercase tracking-wider w-32">바이트수</th>
          </>
        );
      case 'dashboard':
  ...
  const getRowContent = (student: Student) => {
    ...
    switch (tabType) {
      ...
      case 'behavior':
        return (
          <>
            <td className="px-6 py-5 text-sm text-[#4E5968] max-w-[200px] truncate">{student.note || '-'}</td>
            <td className="px-6 py-5 text-sm text-[#4E5968] max-w-[200px] truncate">{student.teacherNote || '-'}</td>
          ...
          <>
            <td className="px-6 py-5 text-center">
              <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold ${
                student.achievement === '우수' ? 'bg-[#E8F3FF] text-[#1B64DA]' : 'bg-[#F2F4F6] text-[#4E5968]'
              }`}>
                {student.achievement}
              </span>
            </td>
      ...
      <div className="toss-card overflow-hidden">
        <table className="w-full text-left border-collapse">
          ...
          <tbody className="divide-y divide-[#F9FAFB]">
            {students.length === 0 ? (
              <tr>
                <td colSpan={colCount} className="py-20 text-center text-[#8B95A1] font-medium">
                  학생 데이터가 없습니다.
                </td>
              </tr>
            ) : (
